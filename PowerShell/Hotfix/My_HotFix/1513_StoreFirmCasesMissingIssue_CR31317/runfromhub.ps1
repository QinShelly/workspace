...
,DB_NAME,siloid,count(1)over() AS Coun from RSI_DIM_SILO where BUILD_NUMBER='$buildnumer' AND type in('S') and RETAILER_NAME='safeway'"
##write-host $sql
...
foreach ($silo in $silos){
	...
	.\1513_StoreFirmCasesMissingIssue_CR31317.ps1 -siloId $siloId -serverName $dbserver -cubeServerName $dbserver  -dbName $dbname -cubeDBName $dbname -cubeName $dbname
