...
foreach ($silo in $silos){
	...
	if ($count -eq 0)
	{
		write-host "$Percent% database $dbname on server $dbserver already applied or the SP not existed"
	} 
	else 
	{
		...
		if ($run)
		{
			write-host "$Percent% job $siloId#UpdatePartition is running. $siloId on server $dbserver"
		} 
		else 
		{
