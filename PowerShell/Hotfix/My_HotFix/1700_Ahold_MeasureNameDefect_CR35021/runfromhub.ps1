...
,DB_NAME,siloid,count(1)over() AS Count from dbo.RSI_DIM_SILO where BUILD_NUMBER='$buildnumer' AND type in('S') and RETAILER_NAME='Ahold'
"
...
foreach ($silo in $silos){
	...
	.\1700_Ahold_MeasureNameDefect_CR35021.ps1 -siloId $siloId -serverName $dbserver -cubeServerName $dbserver  -dbName $dbname -cubeDBName $dbname -cubeName $dbname
	write-host "--$Percent%"
	$Index=$Index+1
}
write-host "finished!"
