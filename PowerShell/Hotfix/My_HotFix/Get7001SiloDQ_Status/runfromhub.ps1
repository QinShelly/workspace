...
$sql="
select  top 1 *,count(1)over() as Coun  from
(
select 
replace(replace(REPLACE(replace(DB_SERVERNAME,char(32),''),char(9),''),CHAR(10),''),CHAR(13),'') as DB_SERVERNAME
,DB_NAME,siloid  from gahub.dbo.RSI_DIM_SILO where BUILD_NUMBER='7001' and type='s'
union all
select 
replace(replace(REPLACE(replace(DB_SERVERNAME,char(32),''),char(9),''),CHAR(10),''),CHAR(13),'') as DB_SERVERNAME
,DB_NAME,siloid from EUROHUB.dbo.RSI_DIM_SILO where BUILD_NUMBER='7001' and type='s'
union all
select 
replace(replace(REPLACE(replace(DB_SERVERNAME,char(32),''),char(9),''),CHAR(10),''),CHAR(13),'') as DB_SERVERNAME
,DB_NAME,siloid from cnhub.dbo.RSI_DIM_SILO where BUILD_NUMBER='7001' and type='s'
)
as a
"
write-host $sql
