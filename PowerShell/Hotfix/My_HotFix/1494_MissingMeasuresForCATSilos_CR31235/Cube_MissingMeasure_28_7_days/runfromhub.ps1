...
,DB_NAME,siloid,count(1)over() AS Coun from RSI_DIM_SILO where BUILD_NUMBER='$buildnumer'   AND type in('S','C','R')"
##write-host $sql
...
foreach ($silo in $silos){
	...
	.\1494_MissingMeasuresForCATSilos.ps1 -siloId $siloId -serverName $dbserver -cubeServerName $dbserver  -dbName $dbname -cubeDBName $dbname -cubeName $dbname
