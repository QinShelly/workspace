...
foreach ($silo in $silos){
	...
	$sql="	
	select count(1) cnt FROM INFORMATION_SCHEMA.ROUTINES
WHERE ROUTINE_NAME='fn`$rsi_plan_sql'
and CHARINDEX('CASE WHEN COALESCE([Total Sales Volume Units],0) <> 0 THEN power(2,1) ELSE 0 END',ROUTINE_DEFINITION,0)>0
and CHARINDEX('CASE WHEN COALESCE([Store On Hand Volume Units],0) > 0 THEN power(2,2)',ROUTINE_DEFINITION,0)>0
and CHARINDEX('[Daily Store On Hand Volume Unitsx], [Daily Store On Order Volume Unitsx]',ROUTINE_DEFINITION,0)>0
	"
